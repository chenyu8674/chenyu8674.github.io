Config.titleText = ["顺序数字广度记忆", "", ""];
Config.contentText = ["屏幕中将会连续出现数字，请连续记住这些数字，并按从前到后的顺序输入它们。", "", ""];
Config.rememberText = ["请记住以下数字", "", ""]; //记忆说明
Config.exerciseText = ["请<tex class='alarm'>顺序</tex>输入刚刚出现的数字", "", ""]; //做题说明
...
var ON_START_PLAY = function(level) {
    currentNumber = 0;
    numberList = [];
    numberCount = Math.floor(1.2 + level * 1.9);
}
