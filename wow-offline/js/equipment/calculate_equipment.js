...
function create_random_equipment_model(param) {
    ...
    model.e_lvl = param.e_lvl ? param.e_lvl : Math.floor(param.c_lvl * multiple_coefficient[model.rare]);
    // 装备词缀
...
function create_static_equipment_model(name) {
    ...
    if (base_model.c_lvl == null && base_model.e_lvl == null) {
        ...
        model.e_lvl = base_model.e_lvl ? base_model.e_lvl : Math.floor(base_model.c_lvl * multiple_coefficient[base_model.rare]);
    }
...
function get_equipment_price(equipment) {
    ...
    base_price *= Math.pow(0.6 + (multiple_coefficient[equipment.rare] - 0.5), 6) / 3;
